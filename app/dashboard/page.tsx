...
export default function DashboardPage() {
  ...
  const family = useMemo(() => profiles.filter((p) => p.userId === user?.id && p.type === 'family'), [profiles, user?.id])
    ...
    <div className="mx-auto max-w-lg px-4 pb-24 pt-6">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <h1 className="mt-1 text-2xl font-bold tracking-tight text-slate-950">Welcome, {user.firstName}</h1>
        </div>
      ...
      <section className="mt-8">
        <h2 className="text-sm font-bold uppercase tracking-[0.14em] text-slate-500">Features</h2>
        <div className="mt-3 grid grid-cols-2 gap-3">
          {['Trusted Venues', 'Promotions', 'Translations', 'Edit Profile'].map((t) => (
